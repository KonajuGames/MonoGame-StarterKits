...
namespace AlienGameSample
{
    ...
    public class GameplayScreen : GameScreen
    {
        ...
        private void SaveHighscore()
        {
            ...
            if (device != null)
            {
				// FIXME: Calling Begin/End is certainly not the
				//        right way to demonstrate usage.
				var asyncResult = device.BeginOpenContainer ("AlienGame", null, null);
				var container = device.EndOpenContainer(asyncResult);
                StreamWriter writer = new StreamWriter(
					container.OpenFile("highscores.txt", FileMode.Create, FileAccess.Write));
        ...
        private void LoadHighscore()
        {
            ...
            if (device != null)
            {
                ...
                if (container.FileExists("highscores.txt"))
                {
                    StreamReader reader = null;

                    try
                    {
                        reader = new StreamReader(container.OpenFile (
							"highscores.txt", FileMode.Open, FileAccess.Read));
